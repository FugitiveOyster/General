...
char char_config[][3] = {
{8,8, 36},    //0	//this is the backspace char
{'2','"', 37},  //1
{'3','#', 41},  //2
{'4','$', 38},  //3
{'5','%', 40},  //4
...
{' ',' ', 48},	//43        //space char
{10, 10, 48}	//44        //carrige return char
};
  ...
  for(int n = 0; n < num_keys; n++){        
    ...
    */
    if(button_state[n] > button_high_thresh && light_state[n] != LED_bright_value){
      ...
      else if(n == shiftLeftKey ){
        keySend = false;
      }
      else if(n == shiftLockKey ){
        keySend = false;
      }
      else{
        keySend = true;
...
void setPWMvlaues(int n, int value){
  ...
  if (keySend == true){                                                                                               
    if (false ==true){
      ...
      Serial.println(n);
      Serial.print(" and char:");
      Serial.println(char_config[pin_config[n][0]][1]);
      Serial.print("shift left is ");
      Serial.println(shiftLeftOn);
      Serial.print("shift right is ");
      Serial.println(shiftRightOn);
      Serial.print("shiftlock is ");
      Serial.println(shiftLockOn);
    
    }
    
    //do caps check

      if (shiftLockOn == true || shiftLeftOn == true || shiftRightOn == true){
  ...
  if (n < 47){
    ...
    //set shiftlock light
    if (shiftLockOn == true){
      tlc.setPWM(pin_config[shiftLockKey][2], 4095);
    }
